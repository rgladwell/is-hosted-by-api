...
object HerokuConfigurationSpec extends Specification {
  ...
  "The Heroku Configuration" should {
    ...
    "return URL from the IS_HOSTED_BY_ASSETS_LOCATION envvar" in new MockEnvironmentVariables(Map("IS_HOSTED_BY_ASSETS_LOCATION" -> "http://example.com")) {
      assetsLocation must_== new URI("http://example.com")
    }

    "throw exception when IS_HOSTED_BY_ASSETS_LOCATION envvar unset" in new MockEnvironmentVariables {
